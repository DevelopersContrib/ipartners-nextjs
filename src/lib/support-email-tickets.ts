...
import { AI_TURN_CAP, IP_SITE } from "@/lib/support-types";
import { queueAiTurn } from "@/lib/support-tickets";
import { postVnocSupportCase } from "@/lib/vnoc-attribution";
import { findSupportMemberByEmail } from "@/lib/support-member";

export { IP_SITE };

const PUBLIC_ID_RE = /\bIP-(\d+)\b/i;
...
async function resolveMember(email: string) {
  return findSupportMemberByEmail(email);
}
...
export async function createContactFormTicket(input: {
  ...
  void sendAppEmail({
    from: ippDefaultFromEmail(),
    fromName: "iPartner Support",
    to: ippDefaultFromEmail(),
    replyTo: email,
    subject: `[iPartner] Contact: ${name} [${publicId}]`,
    text: `Name: ${name}\nEmail: ${email}\nTicket: ${publicId}\n\n${body}`,
...
export async function ingestInboundSupportEmail(input: {
  ...
  if (matchedId) {
    const existing = await prisma.support_tickets.findFirst({
      where: { public_id: matchedId, site: IP_SITE },
    });
