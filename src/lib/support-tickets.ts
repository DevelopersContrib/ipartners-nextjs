...
import { IP_SITE } from "@/lib/support-types";
import {
  SupportTicketError,
  isStatus,
  isCategory,
...
import { findSupportMemberById } from "@/lib/support-member";
...
export async function getPanelTicket(id: number) {
  ...
  let member = null;
  if (ticket.member_id) {
    member = await findSupportMemberById(ticket.member_id);
  }
...
async function escalateTicket(ticketId: number, reason: string, postPublicHandoff: boolean) {
  ...
  if (postPublicHandoff) {
    void notifySupportStaffReply(ticketId, {
      staffName: "iPartner Support Assistant",
    }).catch(() => {});
  }
...
async function handleAiTurn(ticketId: number) {
  ...
  let member = null;
  if (ticket.member_id) {
    const m = await findSupportMemberById(ticket.member_id);
    member = m ? { plan_id: m.plan_id, plan_expiry: m.plan_expiry } : null;
  }
...
async function memberTicketWhere(memberId: number) {
  const member = await findSupportMemberById(memberId);
...
export async function createMemberSupportTicket(input: {
  ...
  const member = await findSupportMemberById(input.memberId);
