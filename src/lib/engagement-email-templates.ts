...
import { IP_DOMAIN_KEY, IP_ENGAGEMENT_CAMPAIGN } from "@/lib/engagement";
...
<div style="margin:0;padding:0;background:#f5f5f4;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;">
  <div style="max-width:560px;margin:0 auto;background:#ffffff;border:1px solid #e7e5e4;border-radius:16px;overflow:hidden;">
    <div style="text-align:center;padding:28px 24px 20px;background:#fff5f5;border-bottom:3px solid #ff646c;">
      <a href="{{siteUrl}}"><img src="${LOGO_URL}" alt="iPartner" width="180" height="auto" style="width:180px;max-width:70%;height:auto;border:0;" /></a>
    </div>
    <div style="padding:28px;">
      ...
      <p style="margin:28px 0 0;font-size:14px;line-height:1.5;color:#78716c;">
        Glad you’re here,<br /><strong style="color:#1c1917;">The iPartner team</strong>
      </p>
    </div>
    <div style="padding:16px 28px;background:#1c1917;color:#a8a29e;font-size:12px;line-height:1.5;">
      <p style="margin:0;">
        <a href="{{siteUrl}}" style="color:#fafaf9;text-decoration:none;font-weight:700;">iPartner</a>
...
/** 10-email feature tour — get partners into each major Referrals surface. */
const FEATURE_TOUR: FeatureEmail[] = [
  {
    ...
    bodyHtml: [
      p(`Hi {{firstname}} — you’re on <strong>iPartner</strong>. Your dashboard is home base for brands, campaigns, rewards, and growth tools.`),
...
const FEATURE_BRIEF = `
Cover these 10 iPartner features in order (one email each) for BRAND MANAGERS:
0 day0 — Dashboard
...
export async function aiImproveActivationEmails(
  ...
  if (aiEnabled()) {
    const prompt = `You are the iPartner lifecycle email agent. Write a 10-email sequence that gets BRAND MANAGERS to check out Referrals features (product tour), not generic hype.

${FEATURE_BRIEF}

Audience: new / free partners running referral programs.
...
stepOrder: 0..9`;
    ...
    const text = await completeText(prompt, {
      maxTokens: 4500,
      system:
        "You write brand-manager product-tour emails for iPartner. Output JSON array only. No markdown.",
    });
