...
import { HY_DOMAIN_KEY, IP_ENGAGEMENT_CAMPAIGN } from "@/lib/engagement";
...
async function aiDraftEmails(
  ...
  const prompt = `Write a 10-email personal engagement sequence for iPartner BRAND MANAGERS.
...
stepOrder: 0..9`;
  ...
  const text = await completeText(prompt, {
    maxTokens: 4500,
    system: "Lifecycle email writer for iPartner partners. Output JSON array only. No markdown.",
  });
  if (!text) return null;
  try {
...
export async function ensureCampaignsForAllSegments(): Promise<{
  ...
  await applyWelcomeEmailTemplates(prisma, HY_DOMAIN_KEY, IP_ENGAGEMENT_CAMPAIGN);
  await prisma.engagement_campaigns.upsert({
    where: {
      domain_key_campaign_key: {
        domain_key: HY_DOMAIN_KEY,
        campaign_key: IP_ENGAGEMENT_CAMPAIGN,
      },
    },
    create: {
      domain_key: HY_DOMAIN_KEY,
