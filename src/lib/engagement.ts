...
import { sendAppEmail, ippDefaultFromEmail } from "@/lib/mail-send";
...
export const IP_ENGAGEMENT_CAMPAIGN = "member_activation";
export const IP_DOMAIN_KEY = "ipartner";
export const RF_DOMAIN_KEY = IP_DOMAIN_KEY; // portable alias
...
export const HY_ENGAGEMENT_CAMPAIGN = IP_ENGAGEMENT_CAMPAIGN;
export const HY_DOMAIN_KEY = IP_DOMAIN_KEY;
...
async function loadUser(userId: number): Promise<UserEngagementProfile | null> {
  ...
  if (!member?.EmailAddress?.trim()) return null;

  const partnershipCount = await prisma.marketPartnership
    .count({ where: { member_id: BigInt(userId) } })
    .catch(() => 0);
  ...
  const email = member.EmailAddress.trim();
  ...
  return {
    ...
    firstname,
    plan: "free",
    projectCount: partnershipCount,
    allowWelcome: true,
    hasTransactionalWelcome: false,
    tokens: {
      unsubscribePageUrl: buildUnsubscribePageUrl(member.MemberId, email),
      listUnsubscribeUrl: buildOneClickUnsubscribeUrl(member.MemberId, email),
    },
  };
}
...
function shouldSkipStep(
  ...
  if (
    enrollment?.campaignKey === IP_ENGAGEMENT_CAMPAIGN &&
    step.stepOrder === 0 &&
...
export async function getRfEngagementBrowse() {
  ...
  const people = recentMembers.map((m) => {
    const e = enrollByUser.get(m.MemberId);
    const name =
    ...
    return {
      ...
      enrolledAt: (e?.enrolled_at ?? m.SignupDate ?? new Date()).toISOString(),
    };
  });
  ...
  const sendMembers = sendUserIds.length
    ? await prisma.members.findMany({
        where: { MemberId: { in: sendUserIds } },
        select: { MemberId: true, EmailAddress: true },
      })
    : [];
  const sendById = new Map(sendMembers.map((m) => [m.MemberId, m]));

  const sends = recentSends.map((s) => {
    const m = sendById.get(s.enrollment.user_id);
    return {
      id: s.id,
      email: m?.EmailAddress || `user #${s.enrollment.user_id}`,
