...
import { emailConfigured, sendAppEmail, ippDefaultFromEmail } from "@/lib/mail-send";
...
export async function notifySupportStaffReply(
  ...
  try {
    ...
    const from = ippDefaultFromEmail();
    ...
    const staffLabel =
      opts?.staffName?.trim() ||
      (fromAgent ? "iPartner Support Assistant" : "iPartner Support");
    ...
    await sendAppEmail({
      from,
      fromName: fromAgent ? "iPartner Support Assistant" : "iPartner Support",
      to: email,
      replyTo: from,
      subject,
      html,
