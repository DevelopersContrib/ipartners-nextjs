...
function buildSegmentWhere(
  ...
  if (rules.inWelcomeSequence === true) {
    clauses.push(
      `EXISTS (SELECT 1 FROM engagement_enrollments e WHERE e.user_id = m.id AND e.domain_key = '${IP_DOMAIN_KEY}' AND e.campaign_key = '${IP_ENGAGEMENT_CAMPAIGN}' AND e.status = 'active')`
    );
  }
  if (rules.notInWelcomeSequence === true) {
    clauses.push(
      `NOT EXISTS (SELECT 1 FROM engagement_enrollments e WHERE e.user_id = m.id AND e.domain_key = '${IP_DOMAIN_KEY}' AND e.campaign_key = '${IP_ENGAGEMENT_CAMPAIGN}')`
    );
  }
      ...
      clauses.push(
        `NOT EXISTS (
          ...
          WHERE e2.user_id = m.id
            AND e2.domain_key = '${IP_DOMAIN_KEY}'
...
export async function getSegmentByKey(segmentKey: string) {
  return prisma.engagement_segments.findUnique({
    where: {
      domain_key_segment_key: { domain_key: IP_DOMAIN_KEY, segment_key: segmentKey },
    },
  });
}

export async function listSegments(domainKey = IP_DOMAIN_KEY): Promise<SegmentRow[]> {
...
export async function deleteSegment(id: number) {
  ...
  if (!row || row.domain_key !== IP_DOMAIN_KEY) throw new Error("Segment not found");
...
export async function aiCreateSegments(): Promise<{
  ...
  if (aiEnabled()) {
    const prompt = `You create audience SEGMENTS for iPartner (referral marketing SaaS). Personal 1:1 engagement — not blasts.
...
Rules must be realistic for activation, nurture, or paid upsell. Prefer free members without campaigns.`;
  ...
  for (const s of proposed) {
    ...
    const existing = await prisma.engagement_segments.findUnique({
      where: { domain_key_segment_key: { domain_key: IP_DOMAIN_KEY, segment_key: key } },
    });
    if (existing) {
