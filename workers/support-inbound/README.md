...
Cloudflare **Email Routing** worker for `support@ipartners.com`.

Parses the message with `postal-mime`, then POSTs to:

`POST https://www.ipartners.com/api/webhooks/support-inbound`
...
1. In Cloudflare → **Email Routing** for `ipartners.com`, enable routing and create a rule:
   - Match: `support@ipartners.com` (or catch-all)
...
4. Smoke test: email `support@ipartners.com` with subject containing `IP-#####` from an open ticket, or a new subject — ticket should appear / update in `/admin/support`.
